...
public static class CampaignsElasticsearchSeeder
{
    ...
    )
    {
        ...
        Log.Information("Seeding Campaigns module entities to Elasticsearch");

        var allDocuments = new List<SearchDocument>();

        Log.Information("Creating search documents for {Count} bloggers", bloggerIds.Count);
        List<SearchDocument> bloggerDocs = await CreateBloggerDocumentsAsync(connection, bloggerIds);
        allDocuments.AddRange(bloggerDocs);

        Log.Information("Creating search documents for {Count} pacts", pactIds.Count);
        List<SearchDocument> pactDocs = await CreatePactDocumentsAsync(connection, pactIds);
        allDocuments.AddRange(pactDocs);

        Log.Information("Creating search documents for {Count} offers", offerIds.Count);
        List<SearchDocument> offerDocs = await CreateOfferDocumentsAsync(connection, offerIds);
        allDocuments.AddRange(offerDocs);

        Log.Information("Creating search documents for {Count} campaigns", campaignIds.Count);
        List<SearchDocument> campaignDocs = await CreateCampaignDocumentsAsync(connection, campaignIds);
        allDocuments.AddRange(campaignDocs);

        Log.Information("Creating search documents for {Count} reviews", reviewIds.Count);
        List<SearchDocument> reviewDocs = await CreateReviewDocumentsAsync(connection, reviewIds);
        allDocuments.AddRange(reviewDocs);

        await IndexDocumentsAsync(searchIndexService, allDocuments);

        Log.Information("Campaigns module Elasticsearch seeding completed");
    }
    ...
    private static async Task<List<SearchDocument>> CreateBloggerDocumentsAsync(
        DbConnection connection,
        List<Guid> bloggerIds
    )
    {
        ...
        foreach (BloggerDataDto blogger in bloggers)
        {
            ...
            string content = $"{blogger.Bio} | Category: {blogger.Category}";

            if (blogger.InstagramUsername is not null)
            {
                content += $" | Instagram: @{blogger.InstagramUsername}";
            }

            if (blogger.InstagramFollowersCount.HasValue)
            {
                content += $" | Followers: {blogger.InstagramFollowersCount}";
            }

            var tags = new List<string> { blogger.Category };
            if (blogger.InstagramUsername is not null)
            {
                tags.Add("Instagram");
            }
            ...
            if (blogger.InstagramUsername is not null)
            {
                metadata["instagramUsername"] = blogger.InstagramUsername;
            }

            if (blogger.InstagramFollowersCount.HasValue)
            {
                metadata["followersCount"] = blogger.InstagramFollowersCount.Value;
            }

            if (blogger.InstagramMediaCount.HasValue)
            {
                metadata["mediaCount"] = blogger.InstagramMediaCount.Value;
            }
    ...
    private static async Task<List<SearchDocument>> CreatePactDocumentsAsync(
        DbConnection connection,
        List<Guid> pactIds
    )
    {
    ...
    private static async Task<List<SearchDocument>> CreateOfferDocumentsAsync(
        DbConnection connection,
        List<Guid> offerIds
    )
    {
    ...
    private static async Task<List<SearchDocument>> CreateCampaignDocumentsAsync(
        DbConnection connection,
        List<Guid> campaignIds
    )
    {
    ...
    private static async Task<List<SearchDocument>> CreateReviewDocumentsAsync(
        DbConnection connection,
        List<Guid> reviewIds
    )
    {
