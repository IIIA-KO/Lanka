...
internal sealed class FacebookService : IFacebookService
{
    ...
    )
    {
        ...
        FacebookAccountsResponse? accountsData =
            await this._facebookApi.GetAccountsAsync(userInfo.Id, queryParams, cancellationToken);
    ...
    )
    {
        ...
        try
        {
            tokenResponse = await this._facebookApi.GetAccessTokenAsync(content, cancellationToken);
        }
    ...
    )
    {
        ...
        try
        {
            debug = await this._facebookApi.GetDebugTokenAsync(
                token.AccessToken,
                token.AccessToken,
                cancellationToken
            );
