    ...
    : IntegrationEventHandler<UserDeletedIntegrationEvent>
{
    ...
    public override async Task Handle(UserDeletedIntegrationEvent integrationEvent, CancellationToken cancellationToken = default)
    {
        Result result = await this._sender.Send(
            new DeleteInstagramAccountDataCommand(integrationEvent.UserId),
            cancellationToken
        );
        
        if (result.IsFailure)
        {
            throw new LankaException(nameof(DeleteInstagramAccountDataCommand), result.Error);
        }
    }
}
